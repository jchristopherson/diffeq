module diffeq_runge_kutta
    ...
    contains
        ...
        procedure, public :: initialize_derivative_storage => &
            rkv_init_fcn_rst_storage
            !! Initializes storage for the function evaluation matrix.
        procedure, public :: initialize_model_storage => rkv_init_model_storage
            !! Initializes storage for the model parameter matrices.
...
! ------------------------------------------------------------------------------
subroutine rkv_init_fcn_rst_storage(this, neqn, err)
    !! Initializes storage for the function evaluation matrix.
    class(rk_variable_integrator), intent(inout) :: this
        !! The rk_variable_integrator object.
    integer(int32), intent(in) :: neqn
        !! The number of ODE's to integrate.
    class(errors), intent(inout), optional, target :: err
        !! An optional errors-based object that if provided 
        !!  can be used to retrieve information relating to any errors 
        !!  encountered during execution. If not provided, a default 
    ...
    if (allocated(this%f)) then
        if (size(this%f, 1) /= neqn .or. size(this%f, 2) /= n) then
            deallocate(this%f)
            allocate(this%f(neqn, n), stat = flag, source = 0.0d0)
            if (flag /= 0) go to 10
        end if
    else
        allocate(this%f(neqn, n), stat = flag, source = 0.0d0)
    ...
    ! Memory Error Handling
10  continue
    call report_memory_error(errmgr, "rkv_init_fcn_rst_storage", flag)
    return
end subroutine

! ------------------------------------------------------------------------------
subroutine rkv_init_model_storage(this, err)
    !! Initializes storage for the model parameter matrices.
    class(rk_variable_integrator), intent(inout) :: this
        !! The rk_variable_integrator object.
    class(errors), intent(inout), optional, target :: err
    ...
    ! Memory Error Handling
10  continue
    call report_memory_error(errmgr, "rkv_init_model_storage", flag)
    return
end subroutine
