module diffeq_multistep
    ...
    !! implementing predictor, build_linear_system,
    !! get_maximum_supported_order, and get_error_constant.
    use iso_fortran_env
    use diffeq_base
    use diffeq_errors
    ...
    type, extends(ode_integrator), abstract :: multistep_integrator
        !! Defines the structure of an implicit, multi-step integrator.
        ...
        integer(int32), private :: m_maxOrder = 5
            ! The user-requested upper limit on the order of the method.
    ...
    contains
        ...
        procedure(build_linearized_system), public, pass, deferred :: build_linear_system
...
pure function mi_effective_max_order(this) result(rst)
    ...
    integer(int32) :: rst
        !! The order limit.
    rst = max(1, min(this%m_maxOrder, this%get_maximum_supported_order()))
end function
...
! --------------------
subroutine mi_push_history(this, x, y)
...
subroutine mi_adjust_order_step(this, yprev, ynew, ek, h)
    ...
    real(real64) :: ekm1, ekp1, r, rbest, safe
    ...
    if (mmax >= 2) then
        ...
        if (k > 1 .and. mmax >= k + 1) then
            errv = this%get_error_constant(k - 1) * &
...
subroutine mi_ode_solver(this, sys, x, iv, args)
    ...
    real(real64), allocatable, dimension(:) :: f, y, yn, fn, ypred, yerr, yi
    ...
    do i = 1, nsteps
        ...
        call this%newton_iteration(sys, h, xo, ypred, yn, converged, args)

        if (.not.converged) then
            ! A failure of the iteration is a statement about the step size
